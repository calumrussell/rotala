...
//Stateless set of rules that the broker uses to validate and execute a trade. Testing for these
//rules is currently within broker implementations where it is easier to check that invalid trades
//fail.
...
impl OrderExecutionRules {
    pub fn run_all<'a>(
        ...
        if let Err(()) = client_has_sufficient_cash(order, price, brkr) {
